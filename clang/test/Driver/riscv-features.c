...
// RUN: not %clang -c --target=riscv64-linux-gnu -gsplit-dwarf -mrelax %s 2>&1 | FileCheck %s --check-prefix=ERR-SPLIT-DWARF
// RUN: not %clang -c --target=riscv64 -gsplit-dwarf=single -mrelax %s 2>&1 | FileCheck %s --check-prefix=ERR-SPLIT-DWARF
// RUN: %clang -### -c --target=riscv64 -mno-relax -g -gsplit-dwarf -mrelax %s 2>&1 | FileCheck %s --check-prefix=SPLIT-DWARF
