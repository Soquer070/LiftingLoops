...
void InnerLoopVectorizer::fixFixedOrderRecurrence(
  ...
  assert(
      PhiR->getNumUsers() == 1 &&
      (RecurSplice->getOpcode() == VPInstruction::FirstOrderRecurrenceSplice ||
       RecurSplice->getOpcode() ==
           VPInstruction::PredicatedFirstOrderRecurrenceSplice) &&
      "recurrence phi must have a single user: FirstOrderRecurrenceSplice or "
...
VectorizationFactor LoopVectorizationPlanner::selectVectorizationFactor(
  ...
  for (const auto &i : VFCandidates) {
    ...
    if (Candidate.Width.isScalable() || !Hints.isFixedVectorizationDisabled()) {
...
SCEV2ValueTy LoopVectorizationPlanner::executePlan(
  ...
  // always jump to exit , but only when the choosen VF by the vectorizer is the
...
createWidenInductionRecipes(PHINode *Phi, Instruction *PhiOrTrunc,
                            ...
                            VFRange &Range, VPValue *EVLRecipe) {
...
VPRecipeBase *VPRecipeBuilder::tryToPredicatedWidenCall(
    CallInst *CI, ArrayRef<VPValue *> Operands, VFRange &Range,
    VPlanPtr &Plan) {
...
std::optional<VPlanPtr> LoopVectorizationPlanner::tryToBuildVPlanWithVPRecipes(
  ...
  if (!VPlanTransforms::adjustFixedOrderRecurrences(*Plan, Builder, EVLRecipe))
    return std::nullopt;
...
void VPWidenEVLRecipe::execute(VPTransformState &State) {
  ...
  // architectures.
  Value *TripCount = State.get(getTripCount(), {0, 0});
